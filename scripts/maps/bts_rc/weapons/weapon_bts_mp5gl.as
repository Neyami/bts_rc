...
namespace weapon_bts_mp5gl
{
    ...
    class weapon_bts_mp5gl : ScriptBasePlayerWeaponEntity, bts_rc_base_weapon
    {
        ...
        void PrimaryAttack()
        {
			...
			else
			{
				if( m_pPlayer.pev.waterlevel == WATERLEVEL_HEAD )
					self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = (self.m_flNextPrimaryAttack = g_Engine.time + 0.104f );
				else
					self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = (self.m_flNextPrimaryAttack = g_Engine.time + 0.104f );
			}
			Fire();
        ...
        void Fire()
        {
            ...
            self.m_flNextPrimaryAttack = g_Engine.time + 0.104f;
			if( m_iFireMode == BURST )
			{
