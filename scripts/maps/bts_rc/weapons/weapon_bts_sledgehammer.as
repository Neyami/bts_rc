/*
 * I wawnnaa be... Sledgehammer!
 */
// Rewrited by Rizulix for bts_rc (january 2025)
// Revisited by AraseFiq._
// Sledgehmamer by SV BOY

namespace weapon_bts_sledgehammer
{
	enum pipe_e
	{
		IDLE1 = 0,
	...
	class weapon_bts_sledgehammer : ScriptBasePlayerWeaponEntity, bts_rc_base_weapon, bts_rc_base_melee
	{
		...
		string GetName()
		{
		  return "weapon_bts_sledgehammer";
		}
		...
		void Spawn()
		{
			...
			g_EntityFuncs.SetModel(self, self.GetW_Model("models/bts_rc/weapons/w_sledgehammer.mdl"));
		...
		bool Deploy()
		{
			return bts_deploy("models/bts_rc/weapons/v_sledgehammer.mdl", "models/bts_rc/weapons/p_sledgehammer.mdl", DRAW, "crowbar", 1, 1.2f);
		}
		...
		void ItemPostFrame()
		{
			if (m_fWhack && (m_pPlayer.pev.button & IN_ATTACK2) == 0)
			{
		     ...
		     // Speed up player attack anim
            // Surely no one will change anim_extensions :clueless:
            if( m_pPlayer.pev.sequence == 21 || m_pPlayer.pev.sequence == 24 || m_pPlayer.pev.sequence == 25 || m_pPlayer.pev.sequence == 28 || m_pPlayer.pev.sequence == 27 || m_pPlayer.pev.sequence == 30) // ref_shoot_crowbar, crouch_shoot_crowbar
		...
		void TertiaryAttack()
		{
			...
			pev.nextthink = g_Engine.time + 1.9f;
		}
		...
		private void ThrowTouch(CBaseEntity@ pOther)
		{
			...
			if ((pev.spawnflags & SF_DODAMAGE) != 0)
			{
			  ...
			  if (pOther.IsBSPModel())
			  {
				...
				g_SoundSystem.EmitSoundDyn(self.edict(), CHAN_VOICE, "bts_rc/weapons/sledge_hitwall2.wav", 1.0f, ATTN_NORM, 0, 80 + Math.RandomLong(0, 29));
			  }
			  else
			  {
				...
				{
				case 0:
				  g_SoundSystem.EmitSoundDyn(self.edict(), CHAN_ITEM, "bts_rc/weapons/sledge_hit1.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM);
				  break;
				case 1:
				  g_SoundSystem.EmitSoundDyn(self.edict(), CHAN_ITEM, "bts_rc/weapons/sledge_hit2.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM);
				  break;
				}
			  }
		  ...
		  if (pOther.IsBSPModel())
			g_SoundSystem.EmitSoundDyn(self.edict(), CHAN_VOICE, "bts_rc/weapons/sledge_hitwall1.wav", 1.0f, ATTN_NORM, 0, 80 + Math.RandomLong(0, 29));
		}
		...
		void SecondaryAttack()
		{
			if (!m_fWhack)
			{
				m_pPlayer.m_flNextAttack = 1.75f; // ( 26.0f / 30.0f );
				self.SendWeaponAnim(ATTACKBIGWIND, 0, pev.body);
				ForceAnimation(25, 28);			  // ref_cock_wrench, crouch_cock_wrench
				m_pPlayer.pev.framerate = 0.45f;
			}
		...
		private bool Swing(bool fFirst)
		{
			...
			if (tr.flFraction >= 1.0f)
			{
				if (fFirst)
				{
					...
					self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + (is_trained_personal ? 1.5f : 2.0f);
					self.m_flTimeWeaponIdle = g_Engine.time + 2.0f;

					// play wiff or swish sound
					g_SoundSystem.EmitSoundDyn(m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sledge_miss1.wav", 1.0f, ATTN_NORM, 0, 94 + Math.RandomLong(0, 0xF));
			...
			else
			{
				...
				g_PlayerFuncs.ScreenShake(m_pPlayer.pev.origin, 3, 75.0, 0.3, 60);
				...
				self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + (is_trained_personal ? 1.1f : 1.6f);
				self.m_flTimeWeaponIdle = g_Engine.time + 2.0f;
				...
				if (self.m_flNextPrimaryAttack + 1.0f < g_Engine.time)
					pEntity.TraceAttack(m_pPlayer.pev, DAMAGE, g_Engine.v_forward, tr, DMG_LAUNCH | DMG_BLAST);		 // first swing does full damage
				else
					pEntity.TraceAttack(m_pPlayer.pev, DAMAGE, g_Engine.v_forward, tr, DMG_LAUNCH | DMG_BLAST); // subsequent swings do 50% (Changed -Sniper) (Half)
				...
				if (pEntity !is null)
				{
					if (pEntity.Classify() != CLASS_NONE && pEntity.Classify() != CLASS_MACHINE && pEntity.BloodColor() != DONT_BLEED)
					{
						...
						{
							case 3:
								g_SoundSystem.EmitSound(m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sledge_hit1.wav", 1.0f, ATTN_NORM);
								break;
							case 2:
								g_SoundSystem.EmitSound(m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sledge_hit2.wav", 1.0f, ATTN_NORM);
								break;
							default:
								g_SoundSystem.EmitSound(m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sledge_hit1.wav", 1.0f, ATTN_NORM);
								break;
				...
				if (fHitWorld)
				{
					...
					{
						case 2:
							g_SoundSystem.EmitSoundDyn(m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sledge_hitwall2.wav", 1.0f, ATTN_NORM, 0, 98 + Math.RandomLong(0, 3));
							break;
						default:
							g_SoundSystem.EmitSoundDyn(m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sledge_hitwall1.wav", 1.0f, ATTN_NORM, 0, 98 + Math.RandomLong(0, 3));
							break;
					}
				}
		...
		private void BigSwing()
		{
			...
			g_SoundSystem.EmitSoundDyn(m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sledge_miss1.wav", Math.RandomFloat(0.98f, 1.0f), ATTN_NORM, 0, 98 + Math.RandomLong(0, 3));
			...
			if (tr.flFraction >= 1.0f)
			{
				...
				self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + 1.85f;
				self.m_flTimeWeaponIdle = g_Engine.time + 2.0f;

				// play wiff or swish sound
				g_SoundSystem.EmitSoundDyn(m_pPlayer.edict(), CHAN_WEAPON, "bts_rc/weapons/sledge_miss2.wav", 1.0f, ATTN_NORM, 0, 94 + Math.RandomLong(0, 0xF));
			...
			else
			{
				...
				self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + 1.64f;
				self.m_flTimeWeaponIdle = g_Engine.time + 2.0f;
