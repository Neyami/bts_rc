		...
		private bool Swing(bool fFirst)
		{
			...
			if (tr.flFraction >= 1.0f)
			{
				if (fFirst)
				{
					...
					self.m_flNextSecondaryAttack = g_Engine.time + (is_trained_personal ? 0.52f : 0.67f);
			...
			else
			{
				...
				self.m_flNextSecondaryAttack = g_Engine.time + (is_trained_personal ? 0.26f : 0.42f);
				self.m_flTimeWeaponIdle = g_Engine.time + 2.0f;
		...
		private bool Shove()
		{
            ...
            else
            {
				...
				// aone
					if( pEntity !is null && ( pEntity.IsPlayer() || pEntity.IsMonster() ) )
					{
						pEntity.pev.velocity = pEntity.pev.velocity +
							( self.pev.origin - pEntity.pev.origin ).Normalize() * -300;
					}
				// end aone
                ...
                if( self.m_flNextPrimaryAttack + 1.0f < g_Engine.time )
                    ...
                    pEntity.TraceAttack( m_pPlayer.pev, DAMAGE * 0.5f, g_Engine.v_forward, tr, DMG_LAUNCH | DMG_CLUB); // subsequent swings do 50% (Changed -Sniper) (Half)
                ...
                if( fHitWorld )
                {
                    ...
                    {
                        case 2:
                            g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "debris/wood2.wav", 1.0f, ATTN_NORM, 0, 94 + Math.RandomLong( 0, 3 ) );
                        break;
                        default:
                            g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "debris/wood2.wav", 1.0f, ATTN_NORM, 0, 94 + Math.RandomLong( 0, 3 ) );
                        break;
