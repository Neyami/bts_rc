...
namespace weapon_bts_python
{
    ...
    class weapon_bts_python : ScriptBasePlayerWeaponEntity, bts_rc_base_weapon
    {
        ...
        bool Deploy()
        {
			...
			if( is_trained_personal == true)
				return bts_deploy( "models/bts_rc/weapons/v_357.mdl", "models/bts_rc/weapons/p_357.mdl", DRAW_QUICK, "python", 3, 0.43f);
			else
				return bts_deploy( "models/bts_rc/weapons/v_357.mdl", "models/bts_rc/weapons/p_357.mdl", DRAW, "python", 3, 0.6f);	
			m_iFireMode == SNOPE;
		}
        ...
        void PrimaryAttack()
        {
			...
			if (m_iFireMode == SNOPE)
				...
				self.SendWeaponAnim( ADS_FIRE, 0, pev.body );
        ...
        void SecondaryAttack()
        {
            if( m_iFireMode == SNOPE )
            {
                m_iFireMode = SCOPE;
				m_pPlayer.SetVModelPos( Vector( 0, 0, 0 ) );
				self.SendWeaponAnim( ADS_IN, 0, pev.body );
            }
            else
            {
                m_iFireMode = SNOPE;
				m_pPlayer.SetVModelPos( Vector( 0, 0, 0 ) );
				self.SendWeaponAnim( ADS_OUT, 0, pev.body );
            }
			ToggleZoom();
			self.m_flTimeWeaponIdle = g_Engine.time + g_PlayerFuncs.SharedRandomFloat( m_pPlayer.random_seed, 5.0f, 10.0f );
            self.m_flNextSecondaryAttack = g_Engine.time + 0.25f;
			self.m_flNextPrimaryAttack = g_Engine.time + 0.75f;
        }
        ...
        void WeaponIdle()
        {
			...
			if (m_iFireMode == SCOPE)
			{
				self.SendWeaponAnim(ADS_IDLE, 0, pev.body);
				self.m_flTimeWeaponIdle = g_Engine.time + 2.93f; // ( 88.0f / 30.0f );
			}
        }
    }
