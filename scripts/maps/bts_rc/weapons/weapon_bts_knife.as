    ...
    class weapon_bts_knife : ScriptBasePlayerWeaponEntity, bts_rc_base_weapon, bts_rc_base_melee
    {
        ...
        private bool Swing( bool fFirst )
        {
            ...
            else
            {
                ...
                if( self.m_flNextPrimaryAttack + 1.0f < g_Engine.time )
                    pEntity.TraceAttack( m_pPlayer.pev, DAMAGE, g_Engine.v_forward, tr, DMG_SLASH ); // first swing does full damage
                else
                    pEntity.TraceAttack( m_pPlayer.pev, DAMAGE * 0.5f /*DAMAGE0*/, g_Engine.v_forward, tr, DMG_SLASH ); // subsequent swings do 50% (Changed -Sniper) (Half)
        ...
        private void BigSwing()
        {
            ...
            else
            {
                ...
                if( self.m_flNextPrimaryAttack + 1.0f < g_Engine.time )
                    pEntity.TraceAttack( m_pPlayer.pev, flDamage, g_Engine.v_forward, tr, DMG_SLASH ); // first swing does full damage
                else
                    pEntity.TraceAttack( m_pPlayer.pev, flDamage * 0.5f, g_Engine.v_forward, tr, DMG_SLASH ); // subsequent swings do 50% (Changed -Sniper) (Half)
